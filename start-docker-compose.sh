...
runServer() {
  ...
  dlv --listen=0.0.0.0:40000 --headless=true --api-version=2 --accept-multiclient exec \
   /server --continue | tee -a /tmp/dlv_log/output.log &
...
buildServer() {
  ...
  go build -gcflags "all=-N -l" -o /server /build/main.go
}
